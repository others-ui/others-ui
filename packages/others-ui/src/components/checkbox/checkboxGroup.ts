...
export class CheckboxGroup<T>
  extends FormItemMixin<typeof BaseElement, unknown[]>(BaseElement)
  implements CheckboxGroupProps<T>
{
  ...
  protected willUpdate(
    state: PropertyValueMap<
      CheckboxGroupProps<T> & { _value: T[]; formItemContext: FormItemContextValue<T[]> }
    >,
  ) {
    watch(state, {
      formItemContext: () => {
