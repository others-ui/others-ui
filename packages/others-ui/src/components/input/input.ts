...
export class Input
  extends FormItemMixin<typeof BaseElement, string>(BaseElement)
  implements InputProps
{
  ...
  protected willUpdate(
    state: PropertyValueMap<
      InputProps & { _value: string; formItemContext: FormItemContextValue<string> }
    >,
  ): void {
    watch(state, {
      formItemContext: () => {
  ...
  render() {
    return html`
      <div class="container">
        ...
        />
        ${this.clearable && !this.disabled && this._value !== undefined && this._value !== ''
          ? html` <button size="small" @click=${this.clearInput}>x</button> `
          : null}
      </div>
    `
  }
